...
//shaderManager.cpp
//--------------------------------------
//Define the Base Shader Methods
...
BaseShader::BaseShader(const BaseShader& src)
{
...
bool BaseShader::Render(ID3D11DeviceContext* context, int index, XMMATRIX& world, XMMATRIX& view, XMMATRIX& projection, ID3D11ShaderResourceView* texture)
{
	bool result;
...
bool BaseShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
void BaseShader::OutputShaderErrorMessage(ID3D10Blob* error, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileError;
	...
	MessageBox(hwnd, L"An Error has occured. See text file for more information", shaderFilename, MB_OK);
}

bool BaseShader::SetShaderParameters(ID3D11DeviceContext* context, XMMATRIX& world, XMMATRIX& view, XMMATRIX& projection)//, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
...
void BaseShader::RenderShader(ID3D11DeviceContext* context, int index)
{
